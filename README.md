...
当前版本是可运行的全栈 MVP，包含：
- 多策略切换：`ema_cross`、`bollinger`、`rsi`、`grid`、`hf_scalp`、`smart_adaptive`
- 回测模块：历史 K 线回放 + 绩效报表 + 参数寻优
- 风控告警：飞书 Webhook 推送
- 部署方式：Docker Compose 一键启动 + 守护运行
...
3. 强烈建议先在 Binance Testnet 联调，再用小资金实盘。

## 8. Conda 启动（可选）

如果你不想用 Docker，也可以使用 Conda 运行本项目：

```bash
conda create -n crypto-quant python=3.12 -y
...
首次在 Windows PowerShell 使用 Conda 时，如果 `conda activate` 不生效，请先执行：

```bash
conda init powershell
```

然后重开终端再执行 `conda activate crypto-quant`。
